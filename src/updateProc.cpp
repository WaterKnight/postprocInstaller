...
UpdateProc::UpdateProc(string targetDir) {
	makeDir(targetDir);
	...
	string host = "http://moonlightflower.net/wc3/postproc/updater/";
	...
	for (map<string,file>::iterator i = localFiles.begin(); i != localFiles.end(); i++) {
		string path = i->first;
		file f = i->second;
	}
	...
	if (pathsToTake.size() > 0) {
		for (unsigned int i = 0; i < pathsToTake.size(); i++) {
			...
			snprintf(c, 100, "downloading files... %.0f%% (%i/%i)\n", i * 100.0f / pathsToTake.size(), i + 1, pathsToTake.size());
